...
* **Implementation:** To enforce mathematical isolation, a new `StandardScaler` instance is instantiated inside every fold loop. It is fitted **exclusively** on the active training slice ('X_tr_trees'), and then applied to transform the validation slice ('X_val_trees') and test matrix ('x_test_encoded').
